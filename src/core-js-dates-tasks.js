...
function getNextFriday(date) {
  ...
  resultDate.setUTCDate(date.getUTCDate() + daysUntilFriday);
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil(((utcDate - startOfYear) / (24 * 60 * 60 * 1000) + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  if (result <= date) {
    result = new Date(date.getFullYear(), date.getMonth() + 1, 13);
  }
  ...
  return getNextFridayThe13th(
    new Date(result.getFullYear(), result.getMonth() + 1, 1)
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const current = new Date(period.start.split('-').reverse().join('-'));
